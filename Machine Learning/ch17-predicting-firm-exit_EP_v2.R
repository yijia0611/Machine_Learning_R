...
#########################################################################################
# slightly modified by Peter Elek
...
data <- read_rds(paste("C:/ElekP/Oktatas/Machine_Learning/Data/bisnode_firms_clean.rds"))
...
create_calibration_plot <- function(data, prob_var, actual_var, y_lab = "Actual event probability" , n_bins = 10, breaks = NULL) {
  ...
  p <- ggplot(data = binned_data) +
    geom_line(aes(mean_prob, mean_actual), size=0.6, show.legend = TRUE) +
    geom_point(aes(mean_prob,mean_actual), size = 1, shape = 16, alpha = 0.7, show.legend=F, na.rm = TRUE) +
    geom_segment(x=min(breaks), xend=max(breaks), y=min(breaks), yend=max(breaks), size=0.3) +
...
createLossPlot <- function(r, best_coords, myheight_small = 5.625, mywidth_small = 7.5) {
  ...
  loss_plot <- ggplot(data = all_coords, aes(x = threshold, y = loss)) +
    ...
    annotate(geom = "text", x = t, y= min(all_coords$loss),
             label=paste0("best threshold: ", round(t,2)), angle=90, vjust = -1, hjust = -0.5, size = 7) +
...
createLossPlot(roc_obj, best_treshold)
